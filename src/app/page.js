...
export default function Home() {
  ...
  const simpanData = (e) => {
    ...
    if (!namaTransaksi || !jumlah) {
      return Swal.fire({ icon: 'warning', title: 'Ups!', text: 'Isi dulu ya Beruang!', confirmButtonColor: '#8E977D' });
    }

    if (editId) {
      setTransaksi(transaksi.map(t => t.id === editId ? { 
        ...t, nama: namaTransaksi, jumlah: Number(jumlah), tipe, sumber: tipe === 'pemasukan' ? sumber : '-' 
      } : t));
      setEditId(null);
      Swal.fire({ icon: 'success', title: 'Berhasil Update!', showConfirmButton: false, timer: 1000 });
    } else {
      const baru = {
        id: Date.now(),
        nama: namaTransaksi,
  ...
  const resetForm = () => {
    setNamaTransaksi(''); setJumlah(''); setSumber(''); setEditId(null);
  };
  ...
  const pemasukan = transaksiTersaring.filter(t => t.tipe === 'pemasukan').reduce((a, b) => a + b.jumlah, 0);
  const pengeluaran = transaksiTersaring.filter(t => t.tipe === 'pengeluaran').reduce((a, b) => a + b.jumlah, 0);
      ...
      <div className="max-w-md mx-auto pb-10">
        ...
        <div className="flex justify-between items-center mb-8 px-2 pt-2">
          <div className="flex items-center gap-4">
             <div className="bg-[#DBCEA5] w-14 h-14 rounded-full flex items-center justify-center text-3xl shadow-inner border-2 border-white/50">🧸</div>
             <div className="flex flex-col">
                <div className="flex items-center gap-2">
                   <h1 className="text-xl font-black leading-none tracking-tight">{userNama || '...'}</h1>
                   <button onClick={() => tanyaNama()} className="opacity-30 hover:opacity-100 transition text-sm">✎</button>
                </div>
                <p className="text-[10px] font-bold opacity-40 uppercase tracking-[0.2em] mt-1.5">Daily Pocket Tracker</p>
             </div>
          </div>
        </div>
        ...
        <div className="bg-[#8A7650] text-[#ECE7D1] p-6 rounded-[2.5rem] mb-8 shadow-2xl relative overflow-hidden">
          <div className="flex justify-between items-center mb-4 relative z-10">
            <button onClick={() => navigasiWaktu('prev')} className="hover:scale-125 transition">◀</button>
            ...
            <button onClick={() => navigasiWaktu('next')} className="hover:scale-125 transition">▶</button>
          </div>
        ...
        <form onSubmit={simpanData} className="bg-[#DBCEA5]/30 p-5 rounded-[2rem] border-2 border-[#DBCEA5] mb-8 space-y-4 shadow-sm">
          ...
          <input 
            ...
            value={namaTransaksi} onChange={(e) => setNamaTransaksi(e.target.value)}
          />
          ...
          {tipe === 'pemasukan' && (
            <input 
              type="text" placeholder="Sumber uang: (Contoh Gajian)" 
              className="w-full bg-[#ECE7D1] p-4 rounded-2xl outline-none border-2 border-[#8E977D]/30 animate-in slide-in-from-top-2"
              value={sumber} onChange={(e) => setSumber(e.target.value)}
            />
          )}

          <div className="flex gap-2">
            <button type="submit" className="flex-[3] bg-[#8E977D] text-[#ECE7D1] p-4 rounded-2xl font-black shadow-lg uppercase tracking-widest text-sm">
              {editId ? 'Update' : 'Simpan'}
            </button>
            {editId && (
              <button type="button" onClick={resetForm} className="flex-1 bg-red-400 text-white p-4 rounded-2xl font-black shadow-lg uppercase text-sm">
                X
              </button>
            )}
          </div>
              ...
              <div key={item.id} className="bg-white/60 backdrop-blur-sm p-4 rounded-[1.5rem] flex justify-between items-center border border-[#DBCEA5] shadow-sm">
                ...
                <div className="text-right">
                  ...
                  <div className="flex gap-3 justify-end mt-1">
                    <button onClick={() => mulaiEdit(item)} className="text-[9px] font-black text-blue-500/60 hover:text-blue-500 uppercase tracking-tighter">Edit</button>
                    <button onClick={() => hapusData(item.id)} className="text-[9px] font-black text-red-400/60 hover:text-red-600 uppercase tracking-tighter">Hapus</button>
                  </div>
                </div>
              </div>
            ))
